 ...
 * 16 cu LSB-first 
 */  
__flash unsigned long crc32_tab_MSBF[256] = { 
...
/* 
 * Func?ia calculeaza CRC-32 folosind metoda clasica bit-cu-bit (fara  
 * tabela) ?i returneaza valoarea finala calculata. 
 */ 
uint32_t crc32wtable(uint32_t init_val_32, uint32_t adr_start,  uint32_t len, enum BitOrder ord){
   
  uint32_t counter; 
  uint32_t crc = init_val_32; // Ini?ializeaza CRC cu valoarea de start 
  for( counter = 0; counter < len; counter++) 
  { 
    if(ord == MSBF) // Varianta cu MSB 
    /* 1. Shift la stânga CRC cu 8 bi?i 
...
/* 
 * Func?ia calculeaza CRC-32 folosind o tabela pre-calculata (lookup table)  
 * ?i returneaza valoarea finala calculata. 
 */  
uint32_t crc32_f(uint32_t polinom32, uint32_t init_val_32,uint32_t adr_start, uint32_t len, enum BitOrder ord){
  uint32_t crc = init_val_32; // Ini?ializeaza CRC cu valoarea de start 
  uint32_t data = 0;          // Variabila temporara pentru octetul curent 
  while(len--) { 
    int i; 
    ...
    { 
      data <<= 24; // Se aliniaza octetul la MSB 
      crc ^= data; // Se transfera ?i integreaza datele în CRC                 
      adr_start++; 
      for(i = 0; i < 8; ++i) { 
        if( crc & 0x80000000) // Se verifica daca MSB este 1 
          crc = (crc << 1) ^ polinom32; 
        else 
          crc = crc << 1; 
      } 
      ...
      crc ^= data; 
      adr_start++; 
      for(i = 0; i < 8; ++i) { // Se verifica daca LSB este 1 
        if(crc & 0x00000001) 
          crc = (crc >> 1) ^ polinom32; 
        else 
          crc = crc >> 1; 
...
{ 
/* 
* Se calculeaza atât CRC-ul folosind func?ia standard crc16(), cât ?i cu  
