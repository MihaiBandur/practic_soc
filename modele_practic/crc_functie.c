...
/* 
 * Func?ia calculeaza CRC-16 folosind o tabela pre-calculata (lookup table)  
 * ?i returneaza valoarea finala calculata. 
 */ 
...
void u16_to_hex(uint16_t value, char *dest){
  dest[0] = nibble_to_hex((value >> 12) & 0x0F);
  dest[1] = nibble_to_hex((value >> 8)  & 0x0F);
  dest[2] = nibble_to_hex((value >> 4)  & 0x0F);
  dest[3] = nibble_to_hex(value & 0x0F);
}



void build_crc_message(uint16_t address_start,
  ...
  u16_to_hex(address_start, &buffer[i]);
  i += 4;
  ...
  u16_to_hex(address_end, &buffer[i]);
  i += 4;
...
void main(){
  ...
  uint16_t address_start = (uint16_t)&min;
  uint16_t address_end   = (uint16_t)&dummy_function;
  uint16_t crc = crc16wtable(0x0000,
                           address_start,
                           address_end - address_start,
                           MSBF);
  
  build_crc_message(address_start,
                  address_end,
                  crc,
                  (char *)data);
