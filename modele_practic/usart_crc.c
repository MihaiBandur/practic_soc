...
void crc16_to_string(uint16_t crc, char *str)
{
    const char hex[] = "0123456789ABCDEF";

    str[0] = hex[(crc >> 12) & 0x0F];
    str[1] = hex[(crc >> 8)  & 0x0F];
    str[2] = hex[(crc >> 4)  & 0x0F];
    str[3] = hex[crc & 0x0F];
    str[4] = '\0';   // terminator de string
}
...
void initialise_usart3(uint16_t baud_rate){
  ...
  UCSR2B |= TRANSMITTER | RECEIVER | set_rx_intrerupt;
...
__interrupt void USART_RX_ISR(void){
  ...
  if(data[index - 1] == '@'){
    ...
    crc_16_for_my_string = crc16wtable(
        0x0000,
        (uint32_t)data,
        index - 1,
        MSBF
    );
    crc16_to_string(crc_16_for_my_string, crc_str);
    data[index -1] = ' ';
    data[index] = 'C';
    data[index + 1] = 'R';
    data[index + 2] = 'C';
    ...
    data[index + 6] = '_';
    data[index + 7] = 'x';
    data[index + 8] = crc_str[0];
    data[index + 9] = crc_str[1];
    data[index + 10] = crc_str[2];
    data[index + 11] = crc_str[3];
    index = 0;
